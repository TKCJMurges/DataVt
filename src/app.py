...
def update_ts_chart(selected_company):
    # Create a copy of the original figure to avoid modifying it
    updated_fig_ts = fig_ts.copy()
    
    # Update visibility of the selected company's trace
    for trace in updated_fig_ts.data:
        if trace.name == selected_company:
            trace.visible = True
        else:
            trace.visible = False

    # Implement logic to update the selected company's data
    selected_company_data = stock_data[stock_data['Name'] == selected_company]
    selected_trace_index = companies.index(selected_company)
    updated_fig_ts.data[selected_trace_index].x = selected_company_data['Date']
    updated_fig_ts.data[selected_trace_index].y = selected_company_data['Closing_Price']

    return updated_fig_ts
...
def update_vwap_chart(selected_company, selected_timeframe):
    ...
    fig_vw = px.line(filtered_stock_data, x='Date', y='VWAP',
    ...
    fig_vw.add_scatter(x=filtered_stock_data['Date'], y=filtered_stock_data['Closing_Price'],
                    mode='markers',
    ...
    fig_vw.update_traces(line=dict(width=1), selector=dict(type='scatter'))  # Increase line width for main plot lines
...
def update_sma_chart(selected_company, selected_timeframe):
    ...
    fig_sm = px.line(filtered_stock_data, x='Date', y=['Closing_Price', 'SMA'],
    ...
    fig_sm.add_scatter(x=filtered_stock_data['Date'], y=filtered_stock_data['Closing_Price'],
                    mode='markers',
    ...
    fig_sm.update_traces(mode='lines', selector=dict(type='scatter'))  # Set lines for main plot lines
...
def update_rsi_chart(selected_company, selected_timeframe):
    ...
    fig_rs = px.line(filtered_stock_data, x=filtered_stock_data['Date'], y=rsi,
    ...
    fig_rs.update_traces(line=dict(color='red', width=2))  # Update line color and width
...
def update_bolinger_chart(selected_company, selected_timeframe):
    ...
    fig_bc.add_trace(go.Scatter(x=filtered_stock_data['Date'], y=filtered_stock_data['Middle_Band'], mode='lines', name='Middle_Band'))
    fig_bc.add_trace(go.Scatter(x=filtered_stock_data['Date'], y=filtered_stock_data['Upper_Band'], mode='lines', name='Upper_Band'))
    fig_bc.add_trace(go.Scatter(x=filtered_stock_data['Date'], y=filtered_stock_data['Lower_Band'], mode='lines', name='Lower_Band'))

    # Add marker for Closing_Price
    fig_bc.add_trace(go.Scatter(x=filtered_stock_data['Date'], y=filtered_stock_data['Closing_Price'], mode='markers', name='Closing_Price', marker=dict(color='red', size=8)))
